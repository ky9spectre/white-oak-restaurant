            ...
            <div className="flex gap-1">
              {[1, 2, 3, 4, 5].map((s) => (
                <Star
                  key={s}
                  size={18}
                  className={s <= Math.round(Number(avgRating)) ? "fill-brass text-brass" : "text-white/20"}
                />
              ))}
            </div>
      ...
      <section className="py-16 px-6 bg-dark">
        <div className="max-w-5xl mx-auto">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
            {reviews.map((r, i) => (
              <div key={r.id} className="bg-white/5 border border-white/10 p-6 relative animate-scaleIn" style={{ animationDelay: `${i * 100}ms` }}>
                <span className="absolute top-3 right-4 text-4xl text-brass/15 font-serif leading-none">&ldquo;</span>
          ...
          <div className="max-w-lg mx-auto">
            <h2 className="font-serif text-2xl mb-6 text-center text-brass">Leave a Review</h2>
            <ReviewForm />
          </div>
        </div>
      </section>
